...
function(ExampleService) {
  ...
  return {
    ...
    controller: ['$scope', '$q', function($scope, $q) {
      ...
      $scope.$on($scope.eventId, function (event, items) {
        if (!$scope.clientPagination) {
          if (items.pagination.count) {
            $scope.bufferSize = items.result.length;
            $scope.bufferResults = items;
            $scope.totalResults = items.pagination.count;
          }
        } else {
          $scope.bufferSize = items.length;
          $scope.bufferResults = items;
          $scope.totalResults = items.length;
        }
      ...
      $scope.goToPage = function (pageNumber) {
        ...
        $q.when(promise, function () {
          ...
          $scope.displayableResults = $scope.clientPagination ?
            ...
            $scope.bufferResults.result.slice(newLowerBound, newUpperBound);
